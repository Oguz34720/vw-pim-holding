...
# --- NameError ve Durum Korumaları İçin Global Değişkenleri Başlatma ---
...
# --- Yararlı Fonksiyonlar (Utility & Core Engines) ---
...
def url_resmini_beyazlat_ve_kaydet(image_url, orijinal_isim, save_folder="Islem_Goren_Resimler"):
    """URL'den resmi çeker, arka planını siler, beyaz fona koyar ve orijinal ismiyle kaydeder."""
...
if missing_packages:
    st.sidebar.warning(f"Eksik Paketler: {', '.join(missing_packages)}. Kurun: pip install {' '.join(missing_packages)}")
...
if st.sidebar.button("API Bağlantısını Sına"):
    if not gemini_key or not genai:
        st.sidebar.error("Eksik API veya Kütüphane.")
    else:
        try:
                    ...
                    if "URUNADI" in df.columns and "STOKKODU" in df.columns:
                        with st.spinner("Motor işliyor... Resimler Shopify CDN yapısına çevriliyor."):
                            for index, row in df.iterrows():
                                ...
                                # Örnek SEO uyumlu resim adı kurgusu
...
with tab3:
    ...
    st.caption("İnternetteki resim linklerini çeker, arka planı bembeyaz stüdyo fonu yapar ve orijinal isimle kaydeder.")
    url_excel = st.file_uploader("İçinde URL ve Resim İsimleri Olan Excel Yükle", type=["xlsx", "xls"], key="url_beyazlatma")
    
    if url_excel:
        url_df = pd.read_excel(url_excel)
        col_url = st.selectbox("Resim Linklerinin Olduğu Sütun", url_df.columns)
        col_isim = st.selectbox("Orijinal Resim İsimlerinin Olduğu Sütun", url_df.columns)
        
        if st.button("Tüm Linklere Sız ve Beyazlat"):
                ...
                for index, row in url_df.iterrows():
                    ...
                    if aktif_url and aktif_isim:
                        ...
                        if kayit_yolu:
                            ...
                            hatali += 1
                            st.warning(f"{aktif_isim} hatası: {durum}")
                            
                st.success(f"🏁 İşlem Bitti! {basarili} resim başarıyla beyazlatıldı. ({hatali} Hata)")
